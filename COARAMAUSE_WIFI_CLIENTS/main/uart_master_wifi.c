...
// BELOW FUNCTION SEND THE SCENE NUMBER TO LCD MODULE !!!!
static void tx_task(void *arg) {
    ...
    while (1) {
        ...
        // DUPLICATION CHECK
        // WE DO NOT WANT SAME SCENE NUMBER TO LCD MODULE !!!
        if(g_sceneNumLast != sceneNumNow) {
          
            // SENDING SCENE NUMBER TO DISPLAY MODULE
            int txResult = sendData(TX_TASK_TAG, g_sceneNumStrNow);
            printf("   ----  tx_task :: DATA SENT !!! \r\n");
            printf("   ----  tx_task :: DATA   %d  BYTES WAS SENT ! \r\n" , txResult );
...
void uart_wifi_SetSceneNumber(char* scnStr) {
    printf("~~~   UART-APP :: uart_wifi_SetSceneNumber() EXECUTED! ||||| %s |||||  WILL BE SENT ! \r\n" , scnStr );
